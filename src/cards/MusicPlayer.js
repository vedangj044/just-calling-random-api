...
const MusicPlayer = () => {
    ...
    const [status, setStatus] = useState({flag : false, message : "Let's play your favourite song.."});
    ...
    return (
        <div className="mx-4 my-8 rounded-md p-4 border-2 border-grey-300 md:px-4">
            ...
            { status.flag ? <audio ref={audioElementRef} src={source} controls className="p-1 pl-0 mt-2" ></audio> : <h1>{status.message}</h1> }
            <br />
            <ContributorInfo
                username="sudo-nick16"
                profilePic="https://avatars.githubusercontent.com/u/73229823?v=4"
            />
            <ApiInfo apiUrl="https://github.com/sudo-nick16/sudoic-server" apiName="Sudoic" />
        </div>
    )
}
