...
const HaikuSyllableCounter = () => {
  ...
  return (
    <div className="mx-4 my-8 rounded-md p-4 border-2 border-grey-300 md:px-4">
      ...
      />
      <button
        className="px-2 py-2 my-2 bg-gray-200 rounded shadow active:bg-gray-300"
        onClick={recalculateSyllables}
      >
        Check Syllables
      </button>
      <div className="mt-2 grid grid-rows-3 gap-2">
        {syllables.map((count, line) => (
          <SyllableCount {...{ count, line: line + 1, key: line }} />
        ))}
      </div>
      <br />
      <ContributorInfo
...
const getSyllables = async (word) => {
  try {
    const res = await axios.get(
      `https://api.datamuse.com/words?sp=${word}&md=s&max=1`
    );
